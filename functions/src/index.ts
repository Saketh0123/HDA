...
function admissionAuthEmail(admissionNumber: string): string {
...
async function allocateNextAdmissionNumber(): Promise<{ admissionNumber: string; sequence: number }> {
  const counterRef = db.collection('counters').doc('admissions');

  const result = await db.runTransaction(async (tx) => {
    const snap = await tx.get(counterRef);
    const data = snap.exists ? (snap.data() as any) : {};
    const current = data?.next as number | null;
    ...
    if (released.length > 0) {
      const sequence = released.shift() as number;
      tx.set(
        counterRef,
        { released, updatedAt: admin.firestore.FieldValue.serverTimestamp() },
        { merge: true }
      );
      return sequence;
    }
...
function requireAdmin(context: any): { uid: string; token: any } {
...
function getAcademicBatch(date = new Date()): string {
  const year = date.getFullYear();
  return `${year}-${year + 1}`;
}

function normalizeAndValidateBatch(input: unknown): string {
  const raw = typeof input === 'string' ? input.trim() : '';
  const value = raw || getAcademicBatch();

  if (!/^\d{4}-\d{4}$/.test(value)) {
    throw new HttpsError('invalid-argument', 'batch must be in format YYYY-YYYY (example: 2025-2026).');
  }
  const [a, b] = value.split('-').map((n) => Number(n));
  if (!Number.isFinite(a) || !Number.isFinite(b) || b !== a + 1) {
    throw new HttpsError('invalid-argument', 'batch must be a valid range like 2025-2026.');
  }
  return value;
}

function nextAcademicBatch(batch: string): string {
...
async function getTransporter() {
  ...
  if (!host || !portRaw || !user || !pass || !from) {
    throw new HttpsError(
      'failed-precondition',
      'Email OTP is not configured. Set SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, SMTP_FROM as Functions secrets.'
    );
  }

  const port = Number(portRaw);
  if (!Number.isFinite(port) || port <= 0) {
    throw new HttpsError('failed-precondition', 'Invalid SMTP_PORT secret.');
  }
...
async function sendMailOrThrow(transporter: Transporter, mail: SendMailOptions): Promise<void> {
  try {
    ...
    if (err?.code === 'EAUTH' || err?.responseCode === 535) {
      throw new HttpsError(
        'failed-precondition',
        'Email service login failed (Gmail rejected credentials). Please check SMTP_USER and SMTP_PASS (Gmail App Password) secrets, then redeploy functions.'
      );
    }

    throw new HttpsError('internal', 'Failed to send OTP email. Please try again later.');
  }
}

export const requestFirstLoginOtp = onCall(
  ...
  if (snap.empty) {
    throw new HttpsError('not-found', 'Admission number not found.');
  }
...
export const completeFirstLogin = onCall(async (request) => {
  ...
  const expiresAt: admin.firestore.Timestamp = codeData.expiresAt;
  ...
  await admin.auth().setCustomUserClaims(userRecord.uid, { role: 'student' satisfies Role });
...
export const resetPasswordWithOtp = onCall(async (request) => {
  ...
  try {
    ...
    if (e?.code === 'auth/user-not-found') {
      throw new HttpsError('failed-precondition', 'Account not created yet. Use first-time login.');
    }
    logger.error('Auth error', e);
...
export const adminUpsertStudent = onCall(async (request) => {
...
export const adminCreateAdmission = onCall(async (request) => {
  const { uid: adminId } = requireAdmin(request);
  ...
  const phoneNumber =
    typeof request.data?.phoneNumber === 'string' && request.data.phoneNumber.trim()
  ...
  if (year !== '1' && year !== '2') {
    throw new HttpsError('invalid-argument', 'year must be "1" or "2".');
  }
  if (!['TECHNICAL', 'EAPCET', 'NDA', 'CEC'].includes(courseOfStudy.toUpperCase())) {
    throw new HttpsError('invalid-argument', 'courseOfStudy must be one of: TECHNICAL, EAPCET, NDA, CEC.');
  }
  ...
  try {
    ...
    try {
      ...
      logger.info('Deleting existing orphan auth user for admission number', { email: authEmail, uid: existingUser.uid });
    ...
    logger.error('Failed to create Auth user for admission, rolling back sequence allocation', e);
    ...
    try {
      ...
      await db.runTransaction(async (tx) => {
        ...
        const data = snap.exists ? (snap.data() as any) : {};
        const releasedRaw = Array.isArray(data?.released) ? data.released : [];
        const released = releasedRaw.map((n: any) => Number(n)).filter((n: any) => Number.isInteger(n) && n > 0);
        ...
        if (sequence === nextVal - 1) {
          ...
          released.sort((a: number, b: number) => a - b);
          tx.set(counterRef, { released, updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
        }
      });
    } catch (rollbackErr) {
      logger.error('Rollback of sequence allocation failed', rollbackErr);
    }
...
export const adminMigrateStudents = onCall(async (request) => {
  const { uid: adminId } = requireAdmin(request);
  ...
  if (!allowedStreams.includes(streamRaw)) {
    throw new HttpsError('invalid-argument', 'stream must be one of: ALL, TECHNICAL, EAPCET, NDA, CEC.');
  }
  if (action !== 'PROMOTE' && action !== 'COMPLETE') {
    throw new HttpsError('invalid-argument', 'action must be PROMOTE or COMPLETE.');
  }
  ...
  for (let i = 0; i < docs.length; i += CHUNK_SIZE) {
    ...
    for (const d of slice) {
      ...
      if (yearValue !== fromYear) continue;
      ...
      if (streamRaw !== 'ALL' && docStream !== streamRaw) continue;
      ...
      const feesUpdate: Record<string, unknown> = {
        year: toYear,
        batch: toBatch,
        lastUpdated: now,
      };
...
export const adminDeleteStudent = onCall(async (request) => {
  const { uid: adminId } = requireAdmin(request);
  ...
  const removed = await db.runTransaction(async (tx) => {
    ...
    const fees = feesSnap.exists ? (feesSnap.data() as any) : null;

    const admissionNumber =
      typeof student?.admissionNumber === 'string' && student.admissionNumber.trim()
    ...
    const pendingAmount =
      typeof fees?.pendingAmount === 'number'
        ? fees.pendingAmount
    ...
    if (typeof admissionSequence === 'number' && Number.isInteger(admissionSequence) && admissionSequence > 0) {
      ...
      if (admissionSequence === nextVal - 1) {
        ...
        const released = releasedRaw
          .map((n: any) => Number(n))
          .filter((n: any) => Number.isInteger(n) && n > 0 && n < newNext);
        released.sort((a: number, b: number) => a - b);
        tx.set(
          counterRef,
          { next: newNext, released, updatedAt: admin.firestore.FieldValue.serverTimestamp() },
          { merge: true }
        );
      } else {
        const releasedRaw = Array.isArray(counterData?.released) ? counterData.released : [];
  ...
  try {
    ...
    if (e?.code !== 'auth/user-not-found' && e?.code !== 'auth/invalid-uid') {
      logger.warn('adminDeleteStudent: failed to delete auth user', { studentId, code: e?.code, message: e?.message });
    }
  }
  ...
  try {
    const appOptions = admin.app().options as { projectId?: string; storageBucket?: string };
    ...
    await admin.storage().bucket(bucketName).file(`students/${studentId}/profile.jpg`).delete({ ignoreNotFound: true } as any);
    photoDeleted = true;
  } catch (e: any) {
    logger.warn('adminDeleteStudent: failed to delete profile photo', { studentId, code: e?.code, message: e?.message });
  }
  ...
  try {
    ...
    logger.warn('adminDeleteStudent: failed to delete logs', { studentId, code: e?.code, message: e?.message });
  }
  ...
  try {
    ...
    if (Number.isFinite(remaining) && remaining === 0) {
      ...
      await db.collection('counters').doc('admissions').set(
        { next: 1, released: [], updatedAt: admin.firestore.FieldValue.serverTimestamp() },
        { merge: true }
      );
    ...
    logger.warn('adminDeleteStudent: failed to reset admin finance data', { code: e?.code, message: e?.message });
  }
...
export const requestEmailVerificationOtp = onCall(
  ...
  async (request) => {
    ...
    if (!existing.empty && existing.docs[0].id !== uid) {
      throw new HttpsError('already-exists', 'Email is already used by another student. Please use a different email.');
    }
...
export const verifyEmailWithOtp = onCall(async (request) => {
  const { uid, token } = requireAuth(request);
  if (token.role !== 'student') {
    throw new HttpsError('permission-denied', 'Student role required.');
  }
  ...
  if ((codeData.emailLower as string | undefined) !== emailLower) {
...
export const requestPasswordResetOtpByEmail = onCall(
  { secrets: [SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, SMTP_FROM] },
  async (request) => {
    const admissionNumber = assertString(request.data?.admissionNumber, 'admissionNumber');
    const email = assertString(request.data?.email, 'email');
    ...
    if (!registeredEmail || registeredEmail !== emailLower) {
      throw new HttpsError(
        'invalid-argument',
        'The email does not match our records for this admission number. Please use your registered email.'
      );
    }
...
export const resetPasswordWithOtpByEmail = onCall(async (request) => {
  const email = assertString(request.data?.email, 'email');
  const emailLower = email.trim().toLowerCase();
  if (!emailLower.includes('@')) {
    throw new HttpsError('invalid-argument', 'email must be a valid email address.');
  ...
  if (password === DEFAULT_STUDENT_PASSWORD) {
    throw new HttpsError('invalid-argument', 'New password cannot be the default password.');
  }
  ...
  await admin.auth().setCustomUserClaims(uid, { role: 'student' satisfies Role });
...
export const adminResetStudentPassword = onCall(async (request) => {
  const { uid: adminId } = requireAdmin(request);
...
export const studentChangePassword = onCall(async (request) => {
  const { uid, token } = requireAuth(request);
  if (token.role !== 'student') {
    throw new HttpsError('permission-denied', 'Student role required.');
  }
...
export const setUserRole = onCall(async (request) => {
  ...
  if (role !== 'admin' && role !== 'student') {
    throw new HttpsError('invalid-argument', 'role must be "admin" or "student".');
  }
...
export const updateStudentMarks = onCall(async (request) => {
  const { uid: adminId } = requireAdmin(request);
  ...
  if (Array.isArray(marks) && marks.length > 0) {
    const sanitized = marks.map((m: any, idx: number) => {
      ...
      if (score > maxScore) {
        throw new HttpsError('invalid-argument', `marks[${idx}].score cannot exceed maxScore.`);
      }
  ...
  if (!subjectOnly || !subjectOnly.trim()) {
    throw new HttpsError('invalid-argument', 'Provide either marks[] or subject/score to update a single subject.');
  }
  ...
  if (score > maxScore) {
    throw new HttpsError('invalid-argument', 'score cannot exceed maxScore.');
  }
  ...
  await db.runTransaction(async (tx) => {
    ...
    const nextMarks = [
      ...prevMarks.filter((m: any) => typeof m?.subject === 'string' && m.subject !== subject),
...
export const addStudentRemark = onCall(async (request) => {
  const { uid: adminId } = requireAdmin(request);
...
export const updateFees = onCall(async (request) => {
  try {
    const { uid: adminId } = requireAdmin(request);
    ...
    if (paidAmount > totalFees) {
      throw new HttpsError('invalid-argument', 'paidAmount cannot exceed totalFees.');
    }
    ...
    await db.runTransaction(async (tx) => {
      ...
      const prev = prevFeesSnap.exists ? (prevFeesSnap.data() as any) : null;
    ...
    logger.error('updateFees failed (unexpected)', {
      message: err?.message,
      name: err?.name,
      code: err?.code,
      stack: err?.stack,
...
export const editPaymentEntry = onCall(async (request) => {
  const { uid: adminId } = requireAdmin(request);
...
export const addStatisticsEntry = onCall(async (request) => {
  ...
  const category =
    typeof request.data?.category === 'string' && request.data.category.trim()
  ...
  if (fromDeposit && studentId) {
    ...
    await db.runTransaction(async (tx) => {
      const feesSnap = await tx.get(feesRef);
      if (!feesSnap.exists) {
        throw new HttpsError('not-found', 'Fees document for student not found.');
      }
      const feesData = feesSnap.data() as any;
      ...
      if (expenditure > currentRemaining) {
        throw new HttpsError('failed-precondition', `Insufficient remaining cautionary deposit. Available: ₹${currentRemaining}`);
      }
...
export const adminUploadProfilePhoto = onCall(async (request) => {
  const { uid: adminId } = requireAdmin(request);
  const studentId = assertString(request.data?.studentId, 'studentId');
  ...
  if (!match || match.length !== 3) {
    throw new HttpsError('invalid-argument', 'Invalid base64 string.');
  }
  ...
  try {
    ...
    if (code === 404 || /bucket does not exist/i.test(msg)) {
      throw new HttpsError(
        'failed-precondition',
        'Firebase Storage is not set up for this project. Open Firebase Console → Storage → Get started (create the default bucket), then try uploading again.'
      );
    }

    console.error('adminUploadProfilePhoto failed', { studentId, adminId, err });
    throw new HttpsError('internal', 'Failed to upload photo. Please try again.');
  }
